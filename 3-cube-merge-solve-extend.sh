...
while [ $# -gt 5 ]; do
    ...
    do 
        ...
        command3="./maplesat-ks/simp/maplesat_static $current_dir/simp/$cube_file$i.adj.simp -no-pre -exhaustive=$current_dir/$n-solve/$v/$i-solve.exhaust -order=$n -minclause >> $current_dir/$n-solve/$v/$i-solve.log"
        command="$command1 && $command2 && $command3"
        echo $command >> $current_dir/$n-solve/$v/solve.commands
        if [ "$p" != "-p" ]
        then
            timeout 200s bash -c "eval $command"
        fi
    done

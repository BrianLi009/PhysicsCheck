        ...
        for f in batch:
            ...
            if len(f.assign) == len(g):
                ...
                """if f.easily_embeddable:
                    ...
                    file = open("embed_result_g6.txt", "a")
...
def determine_embed(g, assignment, graph6_str):
    io = StringIO()
    ...
    io.write('    f = open("embed_result_g6.txt", "a") \n')
    io.write('    f.write(' + ' "  "+ ' + '"' + graph6_str + '" +' + ' "  "  ' + '+' + 'str(s.check()))\n')
    io.write("solve()")
...
def main(G, index):
    """takes in graph in maplesat output format, and order of the graph, g is the graph6 string"""
    #G = nx.from_graph6_bytes(bytes(g, encoding='utf-8'))
    ...
    g = str(nx.to_graph6_bytes(G, header=False))
    determine_embed(graph_dict, assignment, g) #write the file

str_1 = "LuO?HGEA`A@ACJ"
str_2 = "LJaCAOcC_Q?d?f"
#main(str_1, 12)
g = nx.from_graph6_bytes(bytes(str_2, encoding='utf-8'))
print (g.order())
edges = list(g.edges())
for i in range(0, len(edges)):
    new_edges = [x for x in edges if x != edges[i]]
    G = nx.Graph()

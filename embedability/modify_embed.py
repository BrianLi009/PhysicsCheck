    ...
    while batch:
        ...
        for f in batch:
            ...
            if len(f.assign) == len(g):
                ...
                #    return (True, 'easily')
                determine_embed(g, f, count)
                with open('embed_result.txt', 'r') as file:
                    if ('  ' + str(count) + ', ' in file.read()):
...
"""
graph_dict = g6_to_dict("K??????o@_T~")
find_assignments(graph_dict, 2)"""
...
for line in Lines:
    ...
    if count > 0:
        ...
        try:
            find_assignments(graph_dict, str(count))
        except:

...
def check_sphere_embeddability(g, assignment, filename, guess=False):
    ...
    for i in range(len(f.var)):
        io.write(textwrap.dedent("""
            v{i} := {{v{i}c1, v{i}c2, v{i}c3}}; """.format(i=i))) #included
    x = f.var[0]
    y = f.var[1]
    fvars = set()
    try:
    ...
    for i in range(2, len(f.var)):
        ...
        if x in g[f.var[i]]:
            ...
            io.write("\nv{i}c3 := 0;".format(i=i))
            fvars.remove('v%sc3' % i)                       #included
    for i, eq in enumerate(f.eqs):
        io.write(textwrap.dedent("""
            eq{i}z := {eq}; """.format(i=i, eq=assign_to_reduce(eq)))) #included
    had = set()
    nneq = 0
    for v1 in g:
        for v2 in g:
...
graph_dict = g6_to_dict("M_?_Q?P@PPAQb_{@?")
#print (graph_dict)
assignment = find_assignments(graph_dict)
assignment_1 = assignment[1]
print (assignment_1)
check_sphere_embeddability(graph_dict, assignment_1, "2", guess=False)
#print (translation(assignment_1))
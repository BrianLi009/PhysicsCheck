...
def test_minimal(g, order, remove_i):
    Graph = nx.Graph()
    Graph.add_edges_from(maple_to_edges(g, int(order)))
    edge_lst = g.split(" ")[1:-1]
    remove_i = int(remove_i)
    if int(edge_lst[remove_i])>0:
        temp_lst = edge_lst.copy()
        temp_lst[remove_i] = str(-(int(temp_lst[remove_i])))
        string = "a " + " ".join(temp_lst) + " 0"
        edge_lst_tup = maple_to_edges(string, int(order))
        G = nx.Graph()
        G.add_edges_from(edge_lst_tup)
        # could violate 3 constraints, min_degree, triangle, and non_colorable
        check_non_colorable(edge_lst_tup, str(order))
        if check_minimum_degree(G) and check_triangle(G):
            print ("min_degree and triangle satisfied")
            f = open("temp.txt", "a")
            f.write(g + "\n")
            f.close()
        else:
            print ("min_degree or triangle not satisfied")

    #write to two temporary file, if it's in both, g is not minimal, we move on

if __name__ == "__main__":
    test_minimal(sys.argv[1], sys.argv[2], sys.argv[3])
...
"""def test_minimal_2(file, order):
    ...
    for graph in graph_lst:
        ...
        minimum = True
        for gra in graph_lst:
            if graph != gra:
                #print ("checking " + str(graph_lst.index(graph)) + " against " + str(graph_lst.index(gra)))
                gm = isomorphism.GraphMatcher(graph, gra)
        ...
        if minimum == True:
            ...
            output_f = open("minimal_ks_" + str(order) + ".exhaust", "a")
            output_f.write(candidates[graph_lst.index(graph)] + "\n")
            output_f.close()

test_minimal_2("19.exhaust", 19)"""

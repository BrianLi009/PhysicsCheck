...
then
	...
	start=$(date +%s.%N)
	command="./maplesat_static -order=$n -max-exhaustive-var=$((i*(i-1)/2)) -skip-last=$((n-i)) -exhaustive=$dir/$i-solo.exhaust -no-pre $f -keep-blocking=2 -noncanonical-out=$dir/$i-solo.noncanonical $p | tee $dir/subgraph-gen-solo-$i.log"
	echo "$command"
	eval "$command"
	end=$(date +%s.%N)
	...
	start=$(date +%s.%N)
	command="./maplesat_static -order=$n -max-exhaustive-var=$((i*(i-1)/2)) -skip-last=$((n-i)) -exhaustive=$dir/$i.exhaust -no-pre $f -keep-blocking=2 -noncanonical-out=$dir/$i.noncanonical $p > $dir/subgraph-gen-$i.log"
	echo "$command"
	eval "$command"
	end=$(date +%s.%N)
	runtime=$( echo "$end - $start" | bc -l )
	...
	do
		...
		rm "$dir"/"$i".exhaust "$dir"/"$i".noncanonical "$dir"/subgraph-gen-"$i".log 2>/dev/null
		start=$(date +%s.%N)
		command="./maplesat_static -order=$n -max-exhaustive-var=$((i*(i-1)/2)) -skip-last=$((n-i)) -exhaustive=$dir/$i.exhaust -no-pre ${f}_${n}_$((i-1)) -keep-blocking=2 -noncanonical-out=$dir/$i.noncanonical $p > $dir/subgraph-gen-$i.log"
		echo "$command"
		eval "$command"
		end=$(date +%s.%N)
		runtime=$( echo "$end - $start" | bc -l )
